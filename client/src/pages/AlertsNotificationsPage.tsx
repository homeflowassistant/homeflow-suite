...
export default function AlertsNotificationsPage() {
  ...
  const [autoReplyNewCustomerEnabled, setAutoReplyNewCustomerEnabled] =
    useState(true);
  const [autoReplyNewCustomerMessage, setAutoReplyNewCustomerMessage] =
    useState(
      "Thanks for signing up with {{location.name}}! We look forward to keeping your yard clean and enjoyable year-round."
    );

  const [teamNotifyNewLeadEnabled, setTeamNotifyNewLeadEnabled] =
    useState(true);
  const [teamNotifyNewLeadMessage, setTeamNotifyNewLeadMessage] = useState(
    "New Lead Received:\nName: {{name}}\nPhone: {{phone}}\nEmail: {{email}}\nCity: {{address}}\nDogs: {{dogs}}\nFrequency: {{frequency}}\nGive them a call!"
  );

  const [teamNotifyNewCustomerEnabled, setTeamNotifyNewCustomerEnabled] =
    useState(true);
  const [teamNotifyNewCustomerMessage, setTeamNotifyNewCustomerMessage] =
    useState(
  ...
  const [failedPaymentNotifyEnabled, setFailedPaymentNotifyEnabled] =
    useState(true);
  ...
  const [subscriptionPausedNotifyEnabled, setSubscriptionPausedNotifyEnabled] =
    useState(true);
  const [subscriptionPausedNotifyMessage, setSubscriptionPausedNotifyMessage] =
    useState(
  ...
  const isTeamNotifyDisabled =
    !teamNotifyNewLeadEnabled && !teamNotifyNewCustomerEnabled;
  ...
  useEffect(() => {
    ...
    if (d.autoReplyNewLeadMessage)
      setAutoReplyNewLeadMessage(d.autoReplyNewLeadMessage);

    setAutoReplyNewCustomerEnabled(d.autoReplyNewCustomerEnabled);
    if (d.autoReplyNewCustomerMessage)
      setAutoReplyNewCustomerMessage(d.autoReplyNewCustomerMessage);

    setTeamNotifyNewLeadEnabled(d.teamNotifyNewLeadEnabled);
    if (d.teamNotifyNewLeadMessage)
      setTeamNotifyNewLeadMessage(d.teamNotifyNewLeadMessage);

    setTeamNotifyNewCustomerEnabled(d.teamNotifyNewCustomerEnabled);
    if (d.teamNotifyNewCustomerMessage)
      setTeamNotifyNewCustomerMessage(d.teamNotifyNewCustomerMessage);
    ...
    if (d.failedPaymentNotifyMessage)
      setFailedPaymentNotifyMessage(d.failedPaymentNotifyMessage);

    setSkippedJobNotifyEnabled(d.skippedJobNotifyEnabled);
    if (d.skippedJobNotifyMessage)
      setSkippedJobNotifyMessage(d.skippedJobNotifyMessage);

    setSubscriptionPausedNotifyEnabled(d.subscriptionPausedNotifyEnabled);
    if (d.subscriptionPausedNotifyMessage)
      setSubscriptionPausedNotifyMessage(d.subscriptionPausedNotifyMessage);
  }, [settingsQuery.data]);
    ...
    <div className="alerts-page-shell">
      ...
      <section className="alerts-section">
        ...
        <div className="alerts-card-box">
          ...
          <div
            className={`alerts-team-box ${isTeamNotifyDisabled ? "is-disabled" : ""}`}
          >
            <h4 className="alerts-team-box-title">
              Send Team Notifications to:
            </h4>
                ...
                <div className="flex flex-col sm:flex-row items-start gap-4">
                  <div className="pt-2">
                    <ToggleSwitch
                      ...
                      onToggle={() =>
                        setSubscriptionPausedNotifyEnabled(v => !v)
                      }
                    />
                  </div>
                  <div className="flex-1 w-full">
            ...
            <div className="h-full flex flex-col justify-end pt-9">
              <div className="alerts-card-box h-full flex items-center justify-center">
                <div className="alerts-personalize-card">
                  <h3 className="alerts-personalize-title">
                    To personalize message prompts
                  </h3>
                  <button
                    type="button"
        ...
        >
          ...
          >
            <div className="alerts-modal-header">
              <h3 className="alerts-modal-title">
                Personalize Message Prompts
              </h3>
              <button
                type="button"
            ...
            <p className="text-sm text-slate-600 mb-4 leading-relaxed">
              Click any merge tag below to copy it to your clipboard. You can
              paste these into any custom message template to personalize auto
              replies & notifications dynamically.
            </p>
