/**
 * ReactivationQuotePopup Component
 *
 * Dedicated split-panel popup dialog for the Reactivation page.
...
interface ReactivationQuotePopupProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  locationId: string;
  reactivationOption: "Lite" | "Custom Quote & Link";
  onetimeTiming: number;
...
function QuoteFormFields({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-center py-2 bg-blue-100/70 rounded-lg border border-blue-200 mb-3">
        <h3 className="text-sm font-extrabold text-blue-800 uppercase tracking-wider">
          Configure Your Custom Quote (Reactivation)
        </h3>
      </div>
...
// ─── Main Popup Component for Reactivation ───────────────────────────

export default function ReactivationQuotePopup({
  open,
  onOpenChange,
  locationId,
  reactivationOption,
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      await saveMutation.mutateAsync({
        ...
        onetimeTiming: ONETIME_TIMING_LABELS[onetimeTiming] || "Never",
        customQuoteData: {
          ...
          offer2Title: formData.offers[0]?.name || "2 Weeks FREE",
          offer2Description: formData.offers[0]?.description || undefined,
          offer2Image: formData.offers[0]?.image ?? undefined,
      ...
      toast.success("Reactivation settings saved successfully.");
      onSaveSuccess();
      onOpenChange(false);
    } catch (error) {
      ...
      <DialogContent className="max-w-[98vw] w-[98vw] sm:max-w-[96vw] lg:max-w-[92vw] xl:max-w-[1600px] p-0 gap-0 max-h-[95vh] rounded-xl border-2 border-blue-600 flex flex-col">
        ...
        <DialogHeader className="px-6 pt-5 pb-3 bg-slate-50 border-b border-slate-200 flex-shrink-0">
          <DialogTitle className="text-2xl font-bold text-blue-700 mb-2">
            Custom Quote &amp; Link — Reactivation
          </DialogTitle>
