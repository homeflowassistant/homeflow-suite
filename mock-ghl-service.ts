...
(globalThis as any).fetch = async (input: any, init?: any) => {
  ...
  if (url.includes("workers.dev") && url.includes("homeflowassistant")) {
    ...
    return new Response(
      JSON.stringify({
        encrypted_key: "mock-encrypted-" + String(body.api_key ?? "key").slice(0, 6),
  ...
  if (url.includes("services.leadconnectorhq.com")) {
    if (url.includes("/contacts/search")) {
      ...
      if (email === "test@example.com") {
        return new Response(
          JSON.stringify({ contacts: [{ id: "CONTACT_MOCK_1", email, name: "Mock Contact" }], meta: { pageLimit: 100, currentPage: 1, totalPages: 1, total: 1 } }),
          { status: 200, headers: { "content-type": "application/json" } },
        );
      }
      return new Response(JSON.stringify({ contacts: [], meta: { pageLimit: 100, currentPage: 1, totalPages: 0, total: 0 } }), {
        status: 200,
  ...
  if (url.includes("sweepandgo.com")) {
    ...
    return new Response(
      JSON.stringify({ clients: [{ id: 42, email: body.email, first_name: "Mock", last_name: "Client" }], client: { id: 42, email: body.email, name: "Mock S&G Client" } }),
      { status: 200, headers: { "content-type": "application/json" } },
    );
  }
...
export async function getInstallation(locationId: string) {
  if (locationId === "LOC_INSTALLED") {
    return { locationId, expiresAt: Date.now() + 3600_000, accessToken: "mock-token" };
  }
  return null;
}
...
export async function fetchAllCustomValues(_locationId: string, _token: string) {
  return [
...
export async function getLocationCustomValueMap(locationId: string, token: string) {
...
export async function updateExistingCustomValuesOnly(_locationId: string, _updates: Record<string, string>) {
  return undefined;
}

export async function upsertGhlCustomValue(_locationId: string, name: string, value: string) {
  return { id: "new-id", name, value };
}

export async function findCustomValueId(_locationId: string, _token: string, _name: string) {
  return "custom-value-id-1";
}

export async function getCustomFieldIdByName(_locationId: string, _token: string, _name: string) {
...
export async function addTagToContact(_locationId: string, _contactId: string, _tagName: string) {
  return { success: true };
}

export async function removeTagFromContact(_locationId: string, _contactId: string, _tagName: string) {
  return { success: true };
}

export async function uploadToGhlMedia(_locationId: string, _token: string, _file: any) {
